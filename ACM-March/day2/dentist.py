...
with open('./dentist.txt') as data:
    lines = data.readlines()
    
    offices = []
    min_rad = sys.float_info.max
    
    for line in lines[1:]:
        words = line.split(' ')
        words[-1] = words[-1].strip()
        offices.append([int(words[0]), int(words[1])])
        
        for office1 in offices:
            for office2 in offices[1:]:
                
                if office1 == office2:
                    continue
                
                for office3 in offices[2:]:
                    if office1 == office3 or office2 == office3:
                        continue
                    
                    mec_pro = minimum_enclosing_circle([office1, office2, office3])
                    print("Center = {",mec_pro[0][0],",",mec_pro[0][1],"} Radius = ",mec_pro[1])
                    if mec_pro[1] < min_rad:
                        min_rad = mec_pro[1]
    
    print("minimum radius: %f" % min_rad)
    result = min_rad * 2 * pi
    print("minimum circumference : %.2f" % result)